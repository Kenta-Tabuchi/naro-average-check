...
if(201305<=$y.$m&&$y.$m<=date('Ym')){
    ...
    $html_title = '<u><a href="./average.php'.'?type=m&day='.$y.$m.'01'.'">'.$html_title.'</a></u>';
}
...
for ( $day = 1; $day <= $day_count; $day++, $youbi++) {
    ...
    if($y.$m.$day_zero>date('Ymd')||$y.$m<=201304){
        ...
        //リンクを作成
        $week .='<u><a href="./average.php'.'?type=d&day='.$y.$m.$day_zero.'">'.$day_zero.'</a></u>';
        if($youbi % 7 == 2){
            //火曜日の場合週間へのリンクを作成
            $week .='</br>
                    <u><a href="./average.php'.'?type=w&day='.$y.$m.$day_zero.'">週間</a></u>';
        }
        $week .= '</td>'; 
    }
...
<div class= "main">
    ...
    タイトル文字数・単語数／初投稿日から最新投稿まで日数</br>
    話ごとの平均文字数／話ごとの平均更新間隔</br>
    話数割りポイント</br>
    を抽出・計算します。</br>
    </p>
    </br>
    ...
    <div class="container">
        ...
        <?php
            if(201305<=$y.$m&&$y.$m<=date('Ym')){
                ...
                print '<u><a href="./average.php'.'?type=q&day='.$y.$m.'01'.'">四半期</a></u>';
            }else{
                print '<s>四半期</s>';
            }
        ?>
